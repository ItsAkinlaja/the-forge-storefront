...
export async function login(email: string, password: string): Promise<AuthResponse> {
  const result = await forgePost<AuthResponse>("/auth/login", { email, password });
...
export async function register(
  ...
  const result = await forgePost<AuthResponse>("/auth/register", { email, password, firstName, lastName });
  setToken(result.token);
  return result;
}
...
export async function logout(): Promise<void> {
  ...
  if (token) {
    try {
      await forgePost("/auth/logout", {}, token);
    } catch {
...
export async function getMe(token?: string): Promise<AuthUser | null> {
  ...
  if (!t) return null;
  try {
    return await forgeGet<AuthUser>("/auth/me", t);
  } catch {
    clearToken();
    return null;
  }
